...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print("")
            max_a[0] = max(delta_a[0], max_a[0])
            max_a[1] = max(delta_a[1], max_a[1])
            max_a[2] = max(delta_a[2], max_a[2])
            print("  MAX Acceleration: %.3f, %.3f, %.3f" % max_a)
            max_g[0] = max(delta_g[0], max_g[0])
            max_g[1] = max(delta_g[1], max_g[1])
            max_g[2] = max(delta_g[2], max_g[2])
